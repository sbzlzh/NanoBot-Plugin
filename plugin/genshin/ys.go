...
func init() {
	engine := nano.Register("genshin", &ctrl.Options[*nano.Ctx]{
		DisableOnDefault: false,
		Help:             "原神抽卡\n- 原神十连\n- 切换原神卡池",
		PublicDataFolder: "Genshin",
	}).ApplySingle(ctxext.DefaultSingle)

	engine.OnMessageFullMatch("切换原神卡池").SetBlock(true).Limit(ctxext.LimitByUser).
		Handle(func(ctx *nano.Ctx) {
			...
			err := c.SetData(int64(gid), int64(store))
			if err != nil {
				process.SleepAbout1sTo2s()
